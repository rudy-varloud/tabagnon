@if (Session::get('id') == 0)
<script>
    window.location.href = "{{url('/accueil')}}";</script>
@endif
...
<div class='box'>
    ...
    @endphp
    <a href='{{url('/getCompteRendu/'.$uneReunion->idReunion)}}'>Voir le compte rendu</a> pour la {{$uneReunion->typeReunion}} le {{$date->format('d-m-Y')}} à {{$date->format('H:i')}}
    @endforeach
    @endif
</div>
